...
Security Dev Skills 是一个**独立的 skill 仓库**，不依赖外部 skill。只依赖必要的工具和 MCP 服务器。
...
```bash
# 克隆仓库
git clone git@github.com:P0m32Kun/security-dev-skills.git
...
```bash
cat ~/.cache/security-dev-skills/update.log
```
